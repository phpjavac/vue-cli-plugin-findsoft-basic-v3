...
a-form(layout="vertical")
  ...
  a-form-item(label="参考答案")
    Editor(v-model="dataClone.pAnswer[0]",@mounted="onEditorMounted(2, $event)")
...
import {
  Form, InputNumber, Select, Button, Input, Radio, message,
} from 'ant-design-vue';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const answerEditor = reactive({}) as WangEditor;
    const validate = () => {
      if (!stemEditor.txt.text().length) {
        message.error('题目输入字数不满足限制，请重新输入！');
        return false;
      }
      if (!answerEditor.txt.text().length) {
        message.error('参考答案输入字数不满足限制，请重新输入！');
        return false;
      }
    ...
    const onEditorMounted = (index: number, editor: WangEditor) => {
      ...
      if (index === 2) Object.assign(answerEditor, reactive(editor) as WangEditor);
    };
